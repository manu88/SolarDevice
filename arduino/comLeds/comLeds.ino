...
void setup() {
  ...
  strip.setPixelColor(0, intensity1, intensity1, intensity1);
  strip.setPixelColor(0, intensity2, intensity2, intensity2);
  strip.setPixelColor(0, intensity3, intensity3, intensity3);
}
...
void loop() {
  if (Serial.available() >= 3) {
    ...
    if(start == 23){
      int id = Serial.read();
      if (id >=0 && id<nbLeds){
        int inten = Serial.read();
        if (inten != -1){
          sendStatus(id, inten);
          strip.setPixelColor(id, inten, inten, inten);
        }
